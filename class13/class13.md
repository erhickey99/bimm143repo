...
Lets add a log2 fold-change column to out little `meancounts`
data.frame:
...
We want to return both the row and column indices for TRUE values. This
will tell us which genes and samples have 0 counts. This way we can
ignore any genes that have 0 counts in any sample. This way we can focus
on the row answer. The `unique()` ensures no row is counted twice if it
has zero entries for both samples.
...
No because we have not yet accounted for statistical significance of the
differences.
...
What we need from `gage()` is our genes in ENTREZ id format with a
...
Now we can run `gage()` with this input vector abd the geneset we want
...
`pathview()` function. E.g. for “Asthma” I will use pathway.id hsa05310
as seen above.
...
```
    ...
    Info: Working in directory /Users/emilyrosehickey/Desktop/BIMM143/bimm143_github/class13
